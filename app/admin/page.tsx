...
export default async function AdminPage() {
  ...
  // Admin reads intentionally use the service-role client only after the
  // authenticated user's role has been checked server-side.
  ...
  const identity = {
    name: profile.full_name ?? profile.username ?? user.email ?? 'Admin',
    role: profile.role,
  }

  return <main className="main" style={{paddingTop:32}}><div className="topbar"><div><div className="eyebrow">Operations</div><h1>NEXORA Admin.</h1><div className="muted" style={{fontSize:12}}>{identity.name} · {identity.role}</div></div><Link className="btn secondary" href="/app">Back</Link></div>
    ...
    <div className="notice" style={{marginTop:18}}>User ID administrators now authenticate through the standard Supabase session. Admin authorization remains server-side and RLS-compatible.</div>
  </main>
}
