...
export default async function AdminPage() {
  ...
  return <main className="main" style={{paddingTop:32}}><div className="topbar"><div><div className="eyebrow">Operations</div><h1>NEXORA Admin.</h1><div className="muted" style={{fontSize:12}}>{profile.full_name ?? profile.username ?? user.email} · {profile.role}</div></div><Link className="btn secondary" href="/app">Back</Link></div>
    ...
    <section className="glass section" style={{marginTop:18}}><div className="section-head"><div><h3>Manual User ID accounts</h3><div className="muted">Provision administrator-issued credentials without exposing an email address to the user.</div></div><Link className="btn" href="/admin/manual-users">Manage access</Link></div></section>
  </main>
}
