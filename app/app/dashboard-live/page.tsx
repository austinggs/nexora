...
export default async function LiveDashboardPage() {
  ...
  return <>
    ...
    <section className="stats">
      ...
      <div className="glass stat"><div className="muted">Community</div><div className="value">{threads?.length ?? 0}</div><div className="muted" style={{fontSize:11}}>recent threads</div></div>
    </section>
    <div className="grid">
      ...
      <section className="glass section"><div className="section-head"><h3>Recent community</h3><span className="muted">Live</span></div>{(threads ?? []).map(t => <article className="thread" key={t.id}><strong>{t.title}</strong><div className="muted" style={{fontSize:11,marginTop:6}}>{t.profiles?.full_name ?? t.profiles?.username ?? 'Nexorian'}</div><p className="thread-copy">{t.body}</p></article>)}</section>
    </div>
  </>
}
