...
export default async function MiningPage() {
  ...
  const createRigAction = async (_formData: FormData) => { 'use server'; await createRig() }
  const operateRigAction = async (formData: FormData) => { 'use server'; await operateRig(formData) }
  const prestigeRigAction = async (_formData: FormData) => { 'use server'; await prestigeRig() }

  return <>
    <div className="topbar"><div><div className="eyebrow">Mining game</div><h1>Crystal Caverns rig.</h1></div><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><Link className="btn" href="/app/mining/os"><Monitor size={15}/> Open NEXORA OS</Link><Link className="btn secondary" href="/app/mining/catalog"><LibraryBig size={15}/> Hardware catalog</Link><Link className="btn secondary" href="/app">Back</Link></div></div>
    {!rig ? <section className="glass section"><h3>Set up your first rig</h3><p className="muted">Create the starter NEX-01 rig. Components, power budget, thermal state and mining events remain server-authoritative.</p><form action={createRigAction}><button className="btn" type="submit">Create NEX-01</button></form></section> : <>
      <div className="grid" style={{marginTop:0}}>
        <section className="glass section"><div className="section-head"><div><div className="eyebrow">Holographic rig</div><h2 style={{margin:'6px 0'}}>{rig.name}</h2><div className="muted" style={{fontSize:11}}>Click the rig to enter NEXORA OS</div></div><Gauge size={20}/></div><Link href="/app/mining/os" aria-label="Open NEXORA OS from rig"><RigViewer heat={heat} overclocked={overclocked}/></Link></section>
        <section className="glass section"><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><div className="eyebrow">Server-authoritative simulation</div><h3 style={{margin:'6px 0'}}>{overclocked ? 'Overclocked' : 'Stable load'}</h3><div className="muted" style={{fontSize:12}}>{rig.gpu_brand.toUpperCase()} {rig.gpu_model} · {rig.cpu_brand.toUpperCase()} {rig.cpu_model}{rig.motherboard_model ? ` · ${rig.motherboard_model}` : ''}</div></div><Zap size={18}/></div>
          ...
          <div style={{display:'flex',gap:10,flexWrap:'wrap',marginTop:16}}><form action={operateRigAction}><input type="hidden" name="action" value="toggle_overclock"/><button className="btn" type="submit">{overclocked ? 'Disable overclock' : 'Overclock +32%'}</button></form><form action={operateRigAction}><input type="hidden" name="action" value="cool"/><button className="btn secondary" type="submit"><Fan size={15}/> Cool rig</button></form><form action={operateRigAction}><input type="hidden" name="action" value="tick"/><button className="btn secondary" type="submit">Mine next tick</button></form>{gremlins>0&&<form action={operateRigAction}><input type="hidden" name="action" value="defend_gremlin"/><button className="btn" type="submit">Defend gremlin ({gremlinHits}/{gremlins})</button></form>}</div>
