...
import { Activity, Box, Cpu, Globe2, HardDrive, Shield, Store, Users, WalletCards, Wrench, X } from 'lucide-react'

type AppId = 'miner'|'wallet'|'market'|'store'|'facility'|'hardware'|'security'|'guild'|'leaderboard'|'terminal'
...
const apps: {id:AppId;title:string;icon:React.ReactNode}[] = [
 {id:'miner',title:'Miner',icon:<Activity size={18}/>}, {id:'wallet',title:'Wallet',icon:<WalletCards size={18}/>}, {id:'market',title:'Market',icon:<Globe2 size={18}/>}, {id:'store',title:'Hardware Store',icon:<Store size={18}/>}, {id:'hardware',title:'Hardware',icon:<Wrench size={18}/>}, {id:'facility',title:'Facility',icon:<HardDrive size={18}/>}, {id:'security',title:'CyberOps',icon:<Shield size={18}/>}, {id:'guild',title:'Guild',icon:<Users size={18}/>}, {id:'leaderboard',title:'Leaderboard',icon:<Box size={18}/>}, {id:'terminal',title:'NXScript',icon:<Cpu size={18}/>},
]
...
export function MiningDesktop({user,rig,assets,prices,nxa}:{user:{email:string};rig:any;assets:any[];prices:any[];nxa:number}) {
 ...
 const appContent=(id:AppId)=>{
  ...
  if(id==='store') return <div><p className="muted">Components are purchased from the authoritative hardware catalog after explicit confirmation.</p><Link className="btn" href="/app/mining/os/store">Open Store</Link></div>
  if(id==='hardware') return <div><p className="muted">Inspect owned components, health, operating hours and thermal-cycle history. Maintenance restores a component to 100% health.</p><Link className="btn" href="/app/mining/os/hardware">Open Hardware</Link></div>
 ...
 return <main className="os-shell">
  ...
  <div className="os-desktop"><div className="os-grid-overlay" /><div className="os-shortcuts">{apps.map(a=><button key={a.id} className="os-icon" onDoubleClick={()=>{open(a.id);setNextZ(z=>z+1)}}><span>{a.icon}</span><small>{a.title}</small></button>)}</div>
