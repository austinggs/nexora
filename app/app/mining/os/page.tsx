...
export default async function MiningOSPage() {
  ...
  const [{ data: rig }, { data: assets }, { data: prices }, { data: nxa }] = await Promise.all([
  ...
  return <MiningDesktop user={{ email: user.email ?? 'operator' }} rig={rig} assets={assets ?? []} prices={prices ?? []} nxa={Number(nxa ?? 0)} />
}
