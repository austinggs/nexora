...
export default async function WalletPage() {
  ...
  const [{ data: summary }, { data: transactions }, { data: withdrawals }, { data: proofs }] = await Promise.all([
    ...
    supabase.from('payment_proofs').select('id,withdrawal_id,token,amount,tx_hash,status,explorer_url,generated_at').eq('user_id',user.id).order('generated_at',{ascending:false}).limit(10),
  ])
  ...
  return <>
    ...
    <section className="glass section"><div className="section-head"><h3>Payment proof</h3><div className="muted" style={{fontSize:11}}>Generated from confirmed Celo payouts</div></div>{(proofs ?? []).map(proof => <div className="opp" key={proof.id}><div><strong style={{fontSize:13}}>{proof.token} · ${(Number(proof.amount)/100).toFixed(2)}</strong><div className="muted" style={{fontSize:11,marginTop:4}}>{proof.status} · {proof.tx_hash.slice(0,10)}…</div></div><a className="btn secondary" href={proof.explorer_url} target="_blank" rel="noreferrer">View <ExternalLink size={13}/></a></div>)}{(proofs ?? []).length===0 && <div className="muted">No payment proofs yet. Completed payouts generate one automatically.</div>}</section>
  </>
}
