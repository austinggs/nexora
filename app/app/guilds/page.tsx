...
export default async function GuildsPage() {
  ...
  return <main className="main" style={{paddingTop:32}}><div className="topbar"><div><div className="eyebrow">Mining social</div><h1>Guilds & Mega-Blocks.</h1></div><Link className="btn secondary" href="/app">Back</Link></div>
    <section className="glass section"><div className="section-head"><div><h3>Create guild</h3><div className="muted" style={{fontSize:11}}>Guild progress is shared and persistent.</div></div></div><form action={createGuild} style={{display:'flex',gap:8}}><input className="input" name="name" placeholder="Guild name" required/><button className="btn" type="submit">Create</button></form></section>
    <section className="section" style={{padding:0,display:'grid',gap:10}}>{(guilds??[]).map(g=>{const joined=memberMap.has(g.id); const pct=Math.min(100,g.mega_block_threshold ? Number(g.mega_block_progress)/Number(g.mega_block_threshold)*100:0); return <article className="glass section" key={g.id}><div className="section-head"><div><h3>{g.name}</h3><div className="muted" style={{fontSize:11}}>{g.member_count} members · {Number(g.mega_block_progress).toLocaleString()} / {Number(g.mega_block_threshold).toLocaleString()} progress</div></div><span className="badge">{joined?'Member':'Open'}</span></div><div className="meter"><span style={{width:`${pct}%`}} /></div><div className="muted" style={{fontSize:11,marginTop:8}}>Your contribution: {Number(memberMap.get(g.id)??0).toLocaleString()}</div><div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:12}}>{joined ? <><form action={contribute}><input type="hidden" name="guildId" value={g.id}/><input type="hidden" name="amount" value="1000"/><button className="btn" type="submit">Contribute 1,000</button></form><form action={leaveGuild}><input type="hidden" name="guildId" value={g.id}/><button className="btn secondary" type="submit">Leave</button></form></> : <form action={joinGuild}><input type="hidden" name="guildId" value={g.id}/><button className="btn" type="submit">Join guild</button></form>}</div></article>})}</section>
  </main>
}
